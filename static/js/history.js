...
async function fetchDonations() {
    ...
    await processEvents(eventsEthereum, "ethereum", tempNewDonations, web3Ethereum);
    ...
    await processEvents(eventsPolygon, "polygon", tempNewDonations, web3Polygon);
...
async function updateTipHistory(tip) {
    ...
    let fromAccountIdentifier = reverse ? reverse : fromAccount.substring(0, 4).concat("...").concat(fromAccount.substr(-2));

    let amountInUSD = await calculateDollar(tip.tokenAddress, tip.amount, tip.network.toLowerCase());
    let tokenSymbol = getTokenSymbol(tip.network.toLowerCase(), tip.tokenAddress).toUpperCase();
...
async function fetchHistory(web3, tippingContract, network, fromBlock, toBlock, batchSize) {
...
async function init() {
    ...
    await fetchHistory(web3Base, tippingBase, 'base', startBlockBase, currBlockBase, batchSize);
    await fetchHistory(web3Ethereum, tippingEthereum, 'ethereum', startBlockEth, currBlockEthereum, batchSize);
    await fetchHistory(web3Polygon, tippingPolygon, 'polygon', startBlockPolygon, currBlockPolygon, batchSize);
